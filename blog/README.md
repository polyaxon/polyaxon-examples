...
| --- | --- | --- |
| [hyperparameter-seeds](hyperparameter-seeds/) | Local paired seed comparison, a six-configuration random-search matrix with five seeds per child, and downloaded-panel aggregation for **Did you find better hyperparameters—or a lucky seed?** | Python 3.12 and supplied requirements; configured Polyaxon CLI/deployment for matrix execution. Upload with `-u`; install dependencies at startup. Source-reviewed, not executed. |
| [training-recovery](training-recovery/) | CPU PyTorch checkpoint, deliberate interruption, explicit restoration, state comparison, and optional Polyaxon resume/copy workflow for **Resume interrupted training without losing progress**. Article draft pending. | Python 3.12 and a POSIX filesystem locally; configured Polyaxon CLI/deployment and surviving artifact storage for cluster execution. Upload with `-u`; install requirements at startup. Source-reviewed, not executed. |
